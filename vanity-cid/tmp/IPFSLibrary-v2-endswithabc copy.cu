...
int main() {
    ...
    char testContent[100];  // Buffer for random content
    char output[64] = {0};  // Initialize to zero
    int attempts = 0;

    do {
        attempts++;
        int contentLength = rand() % 50 + 10;  // Random length between 10 and 59
        generateRandomContent(testContent, contentLength);
        generateCIDv1((const unsigned char*)testContent, strlen(testContent), output, sizeof(output));
        
        if (attempts % 1000 == 0) {
            printf("Attempts: %d\n", attempts);
        }
    } while (strcmp(output + strlen(output) - 3, "abc") != 0);

    printf("Found CID ending with 'abc' after %d attempts!\n", attempts);
    printf("Content: %s\n", testContent);
    printf("CIDv1: %s\n", output);
